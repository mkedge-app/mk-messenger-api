...
describe('SessionController', () => {
  ...
  describe('create', () => {
    it('should authenticate the user and return a success message', async () => {
      ...
      await sessionController.create(mockRequest as Request, mockResponse as Response);
    ...
    it('should handle incorrect password and return 401 status', async () => {
      ...
      expect(bcrypt.compare).toHaveBeenCalledWith('password', 'hashedPassword');
      expect(mockResponse.status).toHaveBeenCalledWith(401);
      expect(mockResponse.json).toHaveBeenCalledWith({ error: 'Senha incorreta' });
    });
    ...
    it('should handle error and return 500 status', async () => {
      ...
      expect(mockResponse.json).toHaveBeenCalledWith({ error: 'Erro ao autenticar o tenant' });
    });
  });
});
